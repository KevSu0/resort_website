...
class CmsApi {
  ...
  async search<T>(
    ...
    try {
      ...
      const searchResults = allDocsResponse.data.filter((doc: any) =>
    ...
    try {
      ...
      if (response.data.length > 0) {
        ...
        Object.keys(sampleDoc).forEach(field => {
          const values = response.data.map((doc: any) => doc[field]).filter(v => v != null);
        ...
        const docsWithTimestamp = response.data.filter((doc: any) =>
